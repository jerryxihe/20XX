...
#gets the ship_type parameter from the ship select module
def game(ship_type):
    import pygame
    import random
    import time
    from Boss_fight_Lose_Function import lose
    from Boss_fight_Victory_Function import victory
    ...
    while not done:
        ...
        if timer == 95 and wave15 == False:
            ...
            wave15 = True"""
        if timer == 4 and bosswave == False:
            boss.rect.x = 163
            boss.rect.y = 10
            bosswave = True

        #calls the boss laser warning and fires the boss laser every five seconds on the boss wave
        if timer % 5 == 0 and timer > 4 and boss_laser_warning_appear == False:
        ...
        if timer % 5 == 1 and timer > 4 and boss_laser_fired == False:
            boss.laser()
